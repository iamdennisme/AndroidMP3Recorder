...
JNIEXPORT void JNICALL Java_com_czt_mp3recorder_util_LameUtil_init(
	JNIEnv *env, jclass cls, jint inSamplerate, jint inChannel, jint outSamplerate, jint outBitrate, jint quality) {
		try{
		  realInit(env,cls,inSamplerate,inChannel,outSamplerate,outBitrate,quality);
		}catch(...){
		   env->ThrowNew(env->FindClass("java/lang/RuntimeException"),"init 捕获到lame throw native 异常");
		}
}
...
JNIEXPORT jint JNICALL Java_com_czt_mp3recorder_util_LameUtil_encode(
		...
		jint samples, jbyteArray mp3buf) {
	        try{
    		 return realEncode(env,cls,buffer_l,buffer_r,samples,mp3buf);
    		}catch(...){
    		   env->ThrowNew(env->FindClass("java/lang/RuntimeException"),"encode 捕获到lame throw native 异常");
    		   return 0;
    		}
}
...
JNIEXPORT jint JNICALL Java_com_czt_mp3recorder_util_LameUtil_flush(
		JNIEnv *env, jclass cls, jbyteArray mp3buf) {
	      try{
            return realFlush(env,cls,mp3buf);
            }catch(...){
              env->ThrowNew(env->FindClass("java/lang/RuntimeException"),"flush 捕获到lame throw native 异常");
              return 0;
  		}
}

jboolean realWav2Mp3
...
(JNIEnv * env, jobject obj, jstring inputPath, jstring outputPath, jint inSamplerate, jint inChannel, jint outBitrate) {
    try{
       return realWav2Mp3(env,obj,inputPath,outputPath,inSamplerate,inChannel,outBitrate);
    }catch(...){
       env->ThrowNew(env->FindClass("java/lang/RuntimeException"),"wav2Mp3 捕获到lame throw native 异常");
       return JNI_FALSE;
    }
    return JNI_TRUE;
}
...
(JNIEnv *env, jclass cls) {
	try{
	   ...
	   env->ThrowNew(env->FindClass("java/lang/RuntimeException"),"close 捕获到lame throw native 异常");
	}
}
